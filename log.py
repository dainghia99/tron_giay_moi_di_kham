...
FILE_NGUOI_DAN = os.path.join(INPUT_DIR, "data/nam_hinh_2.csv")
...
def read_nguoi_dan(path):
    ...
    persons = []
    bo_qua_trong = []   # dong bi bo qua vi TEN TRONG (co the la dong ke/ghi chu)
    for i in range(data_start, nrows):
        row = raw.iloc[i].tolist()
        name = cell(row, "name")
        if name == "":
            # Ghi lai neu dong VAN co du lieu khac (khong phai dong hoan toan rong)
            if any(str(c).strip() for c in row):
                bo_qua_trong.append(i + 1)  # so dong tren file (1-based)
            continue
    ...
    print(f"   [ĐỌC] Đọc được {len(persons)} người có tên.")
    if bo_qua_trong:
        print(f"   [ĐỌC] ⚠ Bỏ qua {len(bo_qua_trong)} dòng KHÔNG có 'Họ và tên' nhưng vẫn "
              f"có dữ liệu khác (dòng số: {', '.join(map(str, bo_qua_trong))}). "
              f"Kiểm tra xem có phải người bị thiếu tên không.")
    print("─" * 64)
    return persons
...
def _persons_in(block):
    return ([block["head"]] if block["head"] else []) + block["members"]

def _block_xa(block):
    # Lay xa cua HO tu BAT KY nguoi nao con dien xa (khong chi rieng chu ho),
    # tranh rot ca ho chi vi o 'Xa' cua chu ho bi bo trong.
    for p in _persons_in(block):
        if p["xa"].strip():
            return p["xa"]
    return ""

def _in_target(block):
    return norm_ascii(TARGET_XA) in norm_ascii(_block_xa(block))

order_all = list(order)
order = [k for k in order_all if _in_target(hh[k])]

# --- CHAN DOAN: dem du tong nguoi & liet ke ai bi loai o buoc loc xa ---
tong_doc     = sum(len(_persons_in(hh[k])) for k in order_all)
tong_giu     = sum(len(_persons_in(hh[k])) for k in order)
bi_loai_keys = [k for k in order_all if k not in set(order)]
nguoi_bi_loai = [p for k in bi_loai_keys for p in _persons_in(hh[k])]

print("─" * 64)
print(f"CHẨN ĐOÁN SỐ NGƯỜI:")
print(f"   Tổng người gom được (mọi xã) : {tong_doc}")
print(f"   Giữ lại (xã '{TARGET_XA}')     : {tong_giu}")
print(f"   Bị loại do KHÁC/THIẾU xã      : {len(nguoi_bi_loai)}")
if nguoi_bi_loai:
    print("   Danh sách người bị loại (kiểm tra ô 'Xã' của họ):")
    for p in nguoi_bi_loai:
        xa_hien = p["xa"].strip() or "(trống)"
        print(f"      - {p['name']} | năm sinh {p['year'] or '?'} | xã ghi: '{xa_hien}'")
print("─" * 64)
...
for (ten_ban, slug, tong, so_ho, so_kham,
     so_ho_xong, so_ho_chua, khau_ho_xong, khau_ho_chua, so_ho_thieu) in tong_ket:
